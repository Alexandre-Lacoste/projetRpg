...
public class PotionRestController {
	...
	@GetMapping("/{id}")
	@JsonView(Views.ViewPotion.class)
	public Potion find(@PathVariable Long id) {
	...
	@PostMapping("")
	@JsonView(Views.ViewPotion.class)
	public Potion create(@Valid @RequestBody Potion potion, BindingResult result) {
		if(result.hasErrors()) {
			throw new PotionValidationException();
		}
	...
	@PatchMapping("/{id}")
	public Potion partialUpdate(@RequestBody Map<String, Object> updates, @PathVariable Long id) {
		if (!potionRepo.existsById(id)) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Unable to find resource");
		}

		Potion potionFind = potionRepo.findById(id).get();

		potionFind = potionRepo.save(potionFind);
